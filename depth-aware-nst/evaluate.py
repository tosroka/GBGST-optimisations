...
import fast_neural_style_fusion
...
def main():
    ...
    for input_dir in INPUT_DIRS:
        ...
        # create directory
        new_dir = str(args.model).replace('saved_models/', '').replace('.pth','').replace('cvpr_games_oct_30/', '')
        print(new_dir)
    ...
    #     transform = midas_transforms.small_transform
        ...
        for content_image in glob.glob(input_dir + '/*.jpg'): 
            ...
            fast_neural_style_fusion.stylize_p(content_image, args.model, args.cuda, input_dir.replace(scene,''))
