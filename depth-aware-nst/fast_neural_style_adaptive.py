...
from transformer_net_adaptive import TransformerNetAdaptive
...
class NestedFolderDataset(Dataset):
    def __init__(self, root, transform):
        super(NestedFolderDataset, self).__init__()
        paths = []
        self.root = root
...
def train(args):
    ...
    # train_dataset += gta
    content_dataset = FlatFolderDataset(args.dataset, train_transform())
    content_loader = iter(DataLoader(content_dataset, batch_size=args.batch_size, sampler=InfiniteSamplerWrapper(content_dataset)))

    style_dataset = NestedFolderDataset(args.style_dataset, train_transform())
    style_loader = iter(DataLoader(style_dataset, batch_size=args.batch_size, sampler=InfiniteSamplerWrapper(style_dataset)))

    transformer = TransformerNetAdaptive().to(device)
    transformer.train()
    ...
    for e in tqdm(range(args.max_iter)):
        c = next(content_loader).to(device)
        s = next(style_loader).to(device)
        ...
        features_style = vgg(utils.normalize_batch(s))
        gram_style = [utils.gram_matrix(y) for y in features_style]
        style_loss = 0.
        ...
        if (args.feats):
            ...
            x_feat = kornia.feature.hessian_response(c, grads_mode='sobel', sigmas=None)
        ...
        if (e + 1) % args.log_interval == 0:
            if ((not args.depth_loss) and (not args.sem_loss)):
                mesg = "{}\tEpoch {}:\t[{}/{}]\tcontent: {:.6f}\tstyle: {:.6f}\ttotal: {:.6f}".format(
                    time.ctime(), e + 1, count, len(content_dataset),
                ...
                if (args.aim):
                    run.track(agg_content_loss / (e + 1), name='content loss', epoch=e, context={'subset': 'val'})
                    run.track(agg_style_loss / (e + 1), name='style loss', epoch=e, context={'subset': 'val'})
                ...
                mesg = "{}\tEpoch {}:\t[{}/{}]\tcontent: {:.6f}\tstyle: {:.6f}\tdepth: {:.6f}\ttotal: {:.6f}".format(
                    time.ctime(), e + 1, count, len(content_dataset),
                                ...
                                agg_style_loss / (e + 1),
                                agg_depth_loss / (e + 1),
                                (agg_content_loss + agg_style_loss + agg_depth_loss) / (e + 1)
                )
                # writer.add_scalar("content_loss", agg_content_loss / (batch_id + 1), log_count)
                # writer.add_scalar("style_loss", agg_style_loss / (batch_id + 1), log_count)
                ...
                if (args.aim):  
                    run.track(agg_content_loss / (e + 1), name='content loss', epoch=e, context={'subset': 'val'})
                    run.track(agg_style_loss / (e + 1), name='style loss', epoch=e, context={'subset': 'val'})
                    run.track(agg_depth_loss / (e + 1), name='depth loss', epoch=e, context={'subset': 'val'})
                ...
                mesg = "{}\tEpoch {}:\t[{}/{}]\tcontent: {:.6f}\tstyle: {:.6f}\tsemantic: {:.6f}\ttotal: {:.6f}".format(
                    time.ctime(), e + 1, count, len(content_dataset),
                                ...
                                agg_style_loss / (e + 1),
                                agg_sem_loss / (e + 1),
                                (agg_content_loss + agg_style_loss + agg_sem_loss) / (e + 1)
                )
                # writer.add_scalar("content_loss", agg_content_loss / (batch_id + 1), log_count)
                # writer.add_scalar("style_loss", agg_style_loss / (batch_id + 1), log_count)
                ...
                if (args.aim):
                    run.track(agg_content_loss / (e + 1), name='content loss', epoch=e, context={'subset': 'val'})
                    run.track(agg_style_loss / (e + 1), name='style loss', epoch=e, context={'subset': 'val'})
                    run.track(agg_sem_loss / (e + 1), name='semantic loss', epoch=e, context={'subset': 'val'})
                ...
                mesg = "{}\tEpoch {}:\t[{}/{}]\tcontent: {:.6f}\tstyle: {:.6f}\tdepth: {:.6f}\tsemantic: {:.6f}\ttotal: {:.6f}".format(
                    time.ctime(), e + 1, count, len(content_dataset),
                                ...
                                agg_depth_loss / (e + 1),
                                agg_sem_loss / (e + 1),
                                (agg_content_loss + agg_style_loss + agg_depth_loss + agg_sem_loss) / (e + 1)
                )
                # writer.add_scalar("content_loss", agg_content_loss / (batch_id + 1), log_count)
                # writer.add_scalar("style_loss", agg_style_loss / (batch_id + 1), log_count)
                # writer.add_scalar("depth_loss", agg_depth_loss / (batch_id + 1), log_count)
...
def main():
    ...
    train_arg_parser.add_argument("--style-dataset", type=str, required=True,
                                  help="path to styles dataset, the path should point to a folder "
                                       "containing another folder with all the training style images")
    train_arg_parser.add_argument("--max-iter", type=int, default=160000)
