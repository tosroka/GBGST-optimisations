...
class TransformerNetAdaptive(torch.nn.Module):
    def __init__(self):
        super(TransformerNetAdaptive, self).__init__()
    ...
    def forward(self, content, style, alpha=1.0):
        ...
        t = adain(content, style)
        y = alpha * t + (1 - alpha) * content
        # y = self.relu(self.in1(self.conv1(X)))
        # y = self.relu(self.in2(self.conv2(y)))
        # y = self.relu(self.in3(self.conv3(y)))

        cf = self.in1(self.conv1(y))
        cf = self.in2(self.conv2(cf))
        cf = self.in3(self.conv3(cf))
